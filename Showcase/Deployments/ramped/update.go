...
func main() {
	...
	err = yamlv2.Unmarshal(b, &updateData)
	if err != nil {
		panic(err)
	}
	...
	tl, err := template.New("").Parse(string(y))
	if err != nil {
		panic(err)
	}

	writer, err := os.Create("next_deployment.yaml")
	if err != nil {
		panic(err)
	}

	err = tl.Execute(writer, &updateData)
	if err != nil {
		panic(err)
	}

	yamlFilename = "next_deployment.yaml"
